...
oo::class create computer {
  variable program memory pointer relBase \
    ...
    instructions maxX maxY minX minY \

  # Takes the full program as well as the initial input
  # value (0 or 1).
  constructor {programArg {inputArg 1}} {
    ...
    # Track the dimensions of the panel to make it easier to print
    # in the future. This *could* be a struct or something if we
    # wanted to be clean.
    set maxX 0
    set maxY 0
    set minX 0
  ...
  # TODO: Some magic numbers here based off of the maxX, maxY,
  # minX, and minY values. This could be smarter.
  method printPanel {} {
    for {set y 5} {$y >= 0} {incr y -1} {
      for {set x 0} {$x < $maxX} {incr x} {
        set xKey $x
        set yKey [expr {$y - 5}] 
        set key [::math::geometry::p $xKey $yKey]

        if {[dict exists $panelLog $key]} {
          set color [[dict get $panelLog $key] cget -color]
        } else {
          set color 0
        }
  ...
  method move {} {
    ...
    set x [lindex $curLoc 0]
    set y [lindex $curLoc end]
  ...
  # provided color value. If we are on the first panel, it should
  # be white, so give it a value of 1.
  method paintPanel {color} {
    if {[dict size $panelLog] == 0} {
      set panel [panel #auto -coordinates $curLoc -color 1]
    } elseif {[dict exists $panelLog $curLoc]} {
      set panel [dict get $panelLog $curLoc]
