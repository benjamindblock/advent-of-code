...
# Find the position of the paddle on the screen.
proc paddlePos {screen} {
  dict for {k v} [$screen cget -panels] {
    if {$v == 3} {
      return $k
    }
  }
...
# Find the position of the ball on the screen.
proc ballPos {screen} {
  dict for {k v} [$screen cget -panels] {
    if {$v == 4} {
      return $k
...
oo::class create computer {
  ...
  # Takes the full program and # of quarters. 
  constructor {programArg {numQuarters 2}} {
    ...
    # Insert the number of quarters into slot 0 in memory.
    set memory [lreplace $memory 0 0 $numQuarters]
  ...
  method printScreen {} {
    ...
    # Magic numbers 23 and 43 are the maxY and maxX
    # values for the first rendering.
    for {set y 23} {$y >= 0} {incr y -1} {
      for {set x 0} {$x < 43} {incr x} {
    ...
    # Clear screen, render game and score, sleep for 1.
    # This approach should reduce flickering while drawing.
    puts \x1b\[H\x1b\[2J
    puts $game
    puts "Score: [$gameScreen cget -score]"
    sleep 1
  }
  ...
  method perform {structName} {
    ...
    switch $opStruct(opcode) {
      ...
      3 {
        ...
        set paddleX [lindex [split [paddlePos $gameScreen] "_"] 0]
        set ballX [lindex [split [ballPos $gameScreen] "_"] 0]

        # Properly position the paddle beneath the
        # ball
        if {$paddleX > $ballX} {
          set input -1 
  ...
  method draw {x y tileId} {
    if {$x == -1 && $y == 0} {
      $gameScreen configure -score $tileId
    } else {
